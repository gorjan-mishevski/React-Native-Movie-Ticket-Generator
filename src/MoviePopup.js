...
export default class MoviePopup extends Component {
  ...
  state = {
    // Animates slide ups and downs when popup is open or closed
    position: new Animated.Value(this.props.isOpen ? 0 : height),
    // Backdrop opacity
    opacity: new Animated.Value(0),
    // Popup height that can eb changed by pulling it up or down
  ...
  componentWillMount() {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        // Substract delta y from previous height to get new height
        let newHeight = this._previousHeight - dy

        // Animate height change so it looks smooth
        LayoutAnimation.easeInEaseOut()

        // Switch to expand mode if popup pulled up above 80% mark
  ...
  // Handle transitions between re-rendering
  ...
  animateClose() {
    Animated.parallel([
      ...
      // Slide down
      Animated.timing(this.state.position, { toValue: height }) // bottom fo the screen
    ]).start(() => this.setState({
